...
const Home = ({ language, theme }) => {
  ...
  return (
    <main
      id="home"
      className={`flex flex-col items-center justify-center gap-20 ${theme === "light" ? "bg-gray-300" : "bg-[#020211]"
        }`}
    >
      <div className="flex flex-col items-center justify-center gap-16 pt-72 md:pt-96 ">
        <div className="flex w-auto flex-col items-center justify-center gap-1 lg:w-auto ">
          <h1
            className={`text-3xl font-extrabold md:text-5xl lg:text-6xl xl:text-7xl    ${theme === "light" ? "text-gray-950" : "text-gray-200"
              }`}
          >
            Lucas Campos
            <span
              className={`text-3xl font-extrabold md:text-5xl lg:text-6xl xl:text-7xl ${theme === "light" ? "text-purple-600" : "text-purple-900"
                }`}
            >
              .
            </span>
          ...
          <span
            className={`w-auto text-center text-3xl font-extrabold md:w-auto md:text-5xl lg:text-6xl xl:text-7xl  ${theme === "light" ? "text-purple-600" : "text-purple-900"
              }`}
          >
            {typeEffect}
            <Cursor />
        ...
        >
          ...
          <MdOutlineSimCardDownload
            className={`transition-all  duration-300 group-hover:scale-125 ${theme === "light"
              ? ""
      ...
      <section className="flex flex-row gap-10 pb-80">
        {socialNetworks.map((network) => (
          <a
            href={network.href}
            target="_blank"
            className={`all cursor-pointer transition duration-500 hover:-translate-y-1 ${theme === "light" ? "text-black" : "text-gray-200 "
              }`}
            id={network.name}
            key={network.name}
          >
